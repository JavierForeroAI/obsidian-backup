...
⚠️ **CRÍTICO — conditionType:"contact_reply" NO funciona en producción (confirmado 2026-06-28):** GHL acepta el JSON sin error pero en ejecución real **la condición falla silenciosamente** — el bloque IF/ELSE completo no se ejecuta, ni la rama SÍ ni la rama NO. Confirmado con test E2E real (lead de Instagram con "Bogotá" en el mensaje; utm_term quedó vacío incluso para la rama None). **Workaround real que SÍ funciona:** primero usar una acción `update_contact_field` con `{{message.body}}` para copiar el texto a un campo TEXT del contacto (p.ej. `utm_campaign`), y luego evaluar ese campo con `conditionType: "contact_detail"`. El campo recibe el texto, el IF/ELSE sobre `contact_detail` sí evalúa correctamente. Ver [[utm-tracking-avance-general]] (sección DM Instagram v7) para implementación en producción.
